...
class TwoXception(nn.Module):
    def __init__(self, config, num_classes=2):
        ...
        self.xception_base = torch.nn.Sequential(
            *(list(self.xception_base.children())[:-1]))

        if config.genus:
            print("Using two output Xception with {} classes".format(num_classes))
            self.species_fc = nn.Linear(
                in_features=2048, out_features=num_classes[0], bias=True)
            self.genus_fc = nn.Linear(
...
def Mos_Xception(config, model_name="xception", num_classes=2):
    ...
    if model_name == "xceptionconcat":
        ...
        model.last_linear = nn.Linear(
            in_features=5320, out_features=num_classes, bias=True)
