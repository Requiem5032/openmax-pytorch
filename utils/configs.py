...
config.wandb = True
# "Vectech Classification - Network of Experts"
config.wandb_project = "Vectech Classification - Network of Experts"
config.gpu = None
config.fp16 = False
...
# <--paper datasplit location
config.DATA_CSV_PATH = 'data/datasplit_fold{}.csv'.format(FOLD)
...
config.class_map = [(i, config.class_map[i])
                    for i in range(len(config.class_map))]
...
config.blend_params = {
    # range(0.-1.) You can indicate the size of the patched area/s
    'size': .15,
    'alpha': 1.,           # This is used to define a proba distribution
    # This overrides alpha proba distribution. Will fix the % of the image that is modified
    'fixed_proba': 0,
    # Determine if patches may overlap or not. With True they do not overlap
    'grid': True,
    'blend_type': 'cut',   # Options: 'zero', 'noise', 'mix', 'cut', 'random'
    'same_size': False,     # All patches may have the same size or not
    # Cropping patches are from the same subregion as input patches (only with 'mix' and 'cut')
    'same_crop': False,
    # Cropping patches will be from the same or different images (only with 'mix' and 'cut')
    'same_image': False,
}
